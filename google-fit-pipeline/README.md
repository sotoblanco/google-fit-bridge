...
Ran command: `bruin run ./google-fit-pipeline/pipeline/ --full-refresh --start-date 2026-03-01 --end-date 2026-03-29
`


### Data Warehouse Optimization
To ensure optimal query performance and minimize BigQuery costs, the following features have been implemented:

*   **Table Partitioning**: The `staging.fitness_data` and `reports.fitness_daily` tables are partitioned by date. This allows BigQuery to "prune" data and only scan the specific partitions (days) requested by your dashboard, reducing cost and latency.
*   **Data Clustering**: Within each partition, data is clustered by the `data_type` column. This organizes similar records (e.g., all heart rate entries) together on disk for even faster filtering.
*   **Incrementality**: The pipeline uses a `time_interval` materialization strategy, meaning it only processes new data during daily runs rather than recalculating the entire dataset.

### Handling Late-Arriving Data
Due to the nature of wearable health devices (e.g., Xiaomi/Mi Band), heart rate and sleep data can sometimes take 2-3 days to sync to the Google Fit cloud. 

**The Solution**: We implemented a **Backfill Strategy**. When the pipeline is run manually for a specific date range, the SQL logic uses an `ORDER BY extracted_at DESC` and `QUALIFY ROW_NUMBER() = 1` pattern to automatically deduplicate and update records if new, late data arrives.

```bash
# Example: Sweeping the last 14 days to pick up late-arriving watch data
